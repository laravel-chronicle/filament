...
- `ChronicleEntryResource` ViewEntry "External anchoring" section (CHF-18): renders the entry's checkpoint's anchors read-only - per anchor the provider, an `AnchorState` status badge, `anchored_at`, `reference`, and a copyable/truncated `proof` - from stored `CheckpointAnchor.status` only (no provider verification on render). Degrades to "Unanchored" (no checkpoint), "No anchors" (checkpoint with no rows), or "Anchoring not configured" (anchoring disabled). The detail eager-load widens from `checkpoint` to `checkpoint.anchors`, keeping the section N+1-free.
